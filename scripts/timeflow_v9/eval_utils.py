...
CHECKPOINT_PATH = OUTPUT_DIR / "best_flow_v9.pt"
...
def build_model(checkpoint: dict, device: torch.device) -> GWFlowModelV9:
    model = GWFlowModelV9(
...
def build_split_dataset(checkpoint: dict, split_name: str) -> GWDatasetV9:
...
def sample_posterior(model: GWFlowModelV9, waveform: torch.Tensor, num_samples: int, device: torch.device) -> torch.Tensor:
