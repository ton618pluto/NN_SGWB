...
if __package__ is None or __package__ == "":
    ...
    from dataset import GWDatasetV9, PARAMETER_NAMES
    from model import GWFlowModelV9
else:
    from .dataset import GWDatasetV9, PARAMETER_NAMES
    from .model import GWFlowModelV9
...
TRAIN_H1_DIR = SCRIPTS_ROOT / "processed_data_superimposed" / "v1" / "H1_splits"
TRAIN_L1_DIR = SCRIPTS_ROOT / "processed_data_superimposed" / "v1" / "L1_splits"
USE_L1 = True
TRAIN_SAMPLE_STEP = 1
VAL_FRACTION = 0.2
TEST_FRACTION = 0.1
...
BEST_CHECKPOINT_NAME = "best_flow_v9.pt"
LATEST_CHECKPOINT_NAME = "latest_flow_v9.pt"
SPLIT_MANIFEST_NAME = "data_split.json"
EPOCH_LOG_NAME = "epoch_nll_history_v9.txt"
...
def make_loader(dataset: GWDatasetV9, batch_size: int, shuffle: bool, seed: int) -> DataLoader:
...
def save_run_config(train_dataset: GWDatasetV9, train_size: int, val_size: int, test_size: int) -> None:
...
def append_epoch_metrics(output_path: Path, epoch: int, train_loss: float, val_loss: float) -> None:
    output_path.parent.mkdir(parents=True, exist_ok=True)
    file_exists = output_path.exists()
    with output_path.open("a", encoding="utf-8") as file:
        if not file_exists or output_path.stat().st_size == 0:
            file.write("epoch,train_nll,val_nll\n")
        file.write(f"{epoch},{train_loss:.8f},{val_loss:.8f}\n")


def train_epoch(model: GWFlowModelV9, loader: DataLoader, optimizer: optim.Optimizer, scaler: GradScaler, device: torch.device) -> float:
    model.train()
    total_loss = 0.0
...
@torch.no_grad()
def evaluate(model: GWFlowModelV9, loader: DataLoader, device: torch.device, desc: str) -> float:
    model.eval()
    total_loss = 0.0
...
def main() -> None:
    ...
    all_records = GWDatasetV9.build_records(TRAIN_H1_DIR, train_l1_dir, sample_step=TRAIN_SAMPLE_STEP, dataset_name="full_superimposed")
    train_records, val_records, test_records = GWDatasetV9.split_records(all_records, VAL_FRACTION, TEST_FRACTION, SEED)

    train_dataset = GWDatasetV9(records=train_records, dataset_name="train_superimposed")
    val_dataset = GWDatasetV9(records=val_records, label_mean=train_dataset.label_mean, label_std=train_dataset.label_std, dataset_name="val_superimposed")
    test_dataset = GWDatasetV9(records=test_records, label_mean=train_dataset.label_mean, label_std=train_dataset.label_std, dataset_name="test_superimposed")
    ...
    epoch_log_path = OUTPUT_DIR / EPOCH_LOG_NAME
    ...
    for epoch in range(start_epoch, EPOCHS + 1):
        ...
        append_epoch_metrics(epoch_log_path, epoch, train_loss, val_loss)
        elapsed = time.time() - epoch_start
        print(f"train_nll={train_loss:.6f}  val_nll={val_loss:.6f}  lr={optimizer.param_groups[0]['lr']:.2e}  time={elapsed:.1f}s")
        if val_loss < best_val_loss:
            best_val_loss = val_loss
            best_payload = build_checkpoint_payload(model, optimizer, scheduler, scaler, train_dataset, best_val_loss, epoch)
            torch.save(best_payload, best_checkpoint_path)
            print("Saved new best checkpoint.")

        latest_payload = build_checkpoint_payload(model, optimizer, scheduler, scaler, train_dataset, best_val_loss, epoch)
