...
class ResidualConvBlock(nn.Module):
    def __init__(self, in_channels: int, out_channels: int, stride: int = 1, dilation: int = 1, kernel_size: int = 7, groups: int = 8) -> None:
        super().__init__()
...
class GWFlowModelV7(nn.Module):
    def __init__(self, param_dim: int = 5, in_channels: int = 1, context_dim: int = 256, flow_layers: int = 6, flow_hidden_features: int = 256, flow_bins: int = 8, flow_tail_bound: float = 3.0) -> None:
        super().__init__()
        if NFLOWS_IMPORT_ERROR is not None:
            raise ModuleNotFoundError(
                "GWFlowModelV7 requires the 'nflows' package. Install it before running train_flow.py."
