...
CHECKPOINT_PATH = OUTPUT_DIR / "best_flow_v7.pt"
...
def build_model(checkpoint: dict, device: torch.device) -> GWFlowModelV7:
    model = GWFlowModelV7(
...
def build_split_dataset(checkpoint: dict, split_name: str) -> GWDatasetV7:
...
def sample_posterior(model: GWFlowModelV7, waveform: torch.Tensor, num_samples: int, device: torch.device) -> torch.Tensor:
