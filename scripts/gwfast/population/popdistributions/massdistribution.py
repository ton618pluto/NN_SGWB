...
class PowerLawPlusPeak_MassDistribution(MassDistribution):
    ...
    # 初始化模型，设置默认超参数和先验范围，并为双星黑洞质量分布建立参数框架。
    def __init__(self, hyperparameters=None, priorlims_parameters=None):
    ...
    # 这段代码计算双星系统中主天体（质量较大的黑洞）的归一化质量分布概率密度，该分布由幂律背景与高斯峰混合而成，并在低质量端进行平滑截断。
    ...
    # 计算从黑洞（较小质量）质量分布的未归一化核函数，该函数由幂律成分与低质量端平滑锥化函数相乘构成。
    def _mass2_function(self, m2_src, beta_q=None, m_min=None, delta_m=None):
        '''
        Mass distribution of the secondary object.
    ...
    # 计算条件归一化因子，确保在给定主质量m1下，从质量m2的条件概率密度在有效范围内积分为 1。
    ...
    # 计算主质量分布的全局归一化常数，确保整个双星系统质量联合概率密度在定义域内积分为 1。
    ...
    # 计算双星黑洞系统的完整联合质量分布概率密度，通过组合主从质量分布并应用全部归一化约束。
    ...
    # 从模型中随机采样生成双星黑洞的质量对(m1,m2)，用于模拟观测数据。
    def sample_population(self, size, m_min=None, m_max=None):
        '''
        Function to sample the mass model.
    ...
    # 计算联合质量分布对各个超参数的雅可比矩阵（一阶导数），用于梯度优化或敏感性分析。
    ...
    # 计算联合质量分布对各个超参数的海森矩阵（二阶导数），用于误差估计或曲率分析。
