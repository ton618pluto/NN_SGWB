...
def sample_joint_hyperparameters_separate(n_samples=10000, seed=None):
    ...
    zp = rngs["zp"].uniform(0.85, 2.85, n_samples)
    ...
    alpha_m = rngs["alpha_m"].uniform(3.2, 3.6, n_samples)
    m_max = rngs["m_max"].uniform(100.0, 100.0, n_samples)
    delta_m = rngs["delta_m"].uniform(2.0, 10.0, n_samples)
    m_min = rngs["m_min"].uniform(2.0, 2.0, n_samples)
...
if __name__ == "__main__":
    n_samples = 1200
    seed = 42
    output_path = Path(__file__).parent / "joint_hyperparams_train" / "v4" / "joint_hyperparams_train_v4.npz"
