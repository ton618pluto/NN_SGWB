...
OUTPUT_NAME = "val_sample_posterior_heatmap_backup.png"
...
def plot_corner_heatmap(samples: np.ndarray, true_values: np.ndarray, output_path: Path) -> None:
    ...
    for row in range(dim):
        for col in range(dim):
            ...
            if row == col:
                ...
                # 非对角线：原始版本使用 hist2d 画二维热力图。
                # 小方格颜色越深，表示该二维参数区域的后验概率密度越高。
                ax.hist2d(samples[:, col], samples[:, row], bins=36, cmap="Blues")
    ...
    fig.suptitle(
        "Posterior joint distribution (heatmap backup version)\n"
...
def main() -> None:
    ...
    print(f"Saved heatmap backup figure: {output_path}")
