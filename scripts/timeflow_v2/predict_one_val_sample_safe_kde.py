...
# KDE 图一般建议比普通 contour 多一些样本；1200 是兼顾平滑度和速度的折中值。
...
OUTPUT_NAME = "val_sample_posterior_kde_safe.png"
...
def draw_kde_contour(ax, x: np.ndarray, y: np.ndarray) -> None:
    # 这里使用 Gaussian KDE 来画更平滑的二维联合后验图，更接近论文风格。
    ...
    try:
        ...
        # 如果 KDE 因为协方差退化等原因失败，就退回到 histogram2d contour。
...
def plot_corner_kde(samples: np.ndarray, true_values: np.ndarray, output_path: Path) -> None:
    ...
    fig.suptitle(
        "Posterior joint distribution (KDE version, new model)\n"
