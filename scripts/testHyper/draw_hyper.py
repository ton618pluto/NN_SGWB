...
def sample_joint_hyperparameters_separate(n_samples=10000, seed=None):
    ...
    # 2. 计算每个样本对应的 lambda 上限 (依据公式 6: lambda <= (m_min/m_max)^(alpha-1))
    # 注意：物理上 alpha 通常大于 1，如果 alpha_m < 1，上限可能会变得很大
    lambda_upper_bound = (m_min / m_max) ** (alpha_m - 1)

    # 3. 确保上限在合理范围内（例如不要超过 1.0，防止 alpha-1 为负数时结果爆炸）
    lambda_upper_bound = np.clip(lambda_upper_bound, 0, 0.18)

    # 4. 生成 lambda_peak，每个样本的取值范围是 [0, lambda_upper_bound]
    # np.random.random(n_samples) 生成 0 到 1 之间的随机数，乘以高度即得范围内随机值
    lambda_peak = np.random.random(n_samples) * lambda_upper_bound

    # 如果你想保留之前 [0.07, 0.18] 的硬性范围作为额外约束，可以这样做：
# lambda_peak = np.random.uniform(0.0, np.minimum(0.18, lambda_upper_bound), n_samples)
...
if __name__ == "__main__":
    ...
    # 保存为 npz 文件
    save_path = "./joint_hyperparams_train/joint_hyperparams_train.npz"
    np.savez(save_path,
