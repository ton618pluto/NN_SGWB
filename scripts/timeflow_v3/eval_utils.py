...
from dataset import GWDatasetV3
from model import GWFlowModelV3


CURRENT_DIR = Path(__file__).resolve().parent
OUTPUT_DIR = CURRENT_DIR / "outputs"
CHECKPOINT_PATH = OUTPUT_DIR / "best_flow_v3.pt"
...
def load_records_from_manifest(path: Path) -> tuple[list[dict], list[dict]]:
    manifest = load_json(path)
    train_records = [_deserialize_record(item) for item in manifest["train_records"]]
    val_records = [_deserialize_record(item) for item in manifest["val_records"]]
...
def load_run_artifacts(device: torch.device) -> tuple[dict, dict, dict]:
    checkpoint = load_checkpoint(CHECKPOINT_PATH, device)
    run_config = load_json(RUN_CONFIG_PATH)
    split_manifest = load_json(SPLIT_MANIFEST_PATH)
    return checkpoint, run_config, split_manifest


def build_val_dataset(checkpoint: dict) -> GWDatasetV3:
    _, val_records = load_records_from_manifest(SPLIT_MANIFEST_PATH)
    use_l1 = int(checkpoint["num_channels"]) == 2
    if use_l1:
        missing_l1 = [record["h1"] for record in val_records if record["l1"] is None]
        if missing_l1:
            raise RuntimeError(
                "Validation split manifest does not contain usable L1 paths for a 2-channel model. "
                f"First bad record: {missing_l1[0]}"
            )
    else:
        val_records = [{"h1": record["h1"], "l1": None} for record in val_records]

    return GWDatasetV3(
        records=val_records,
        label_mean=checkpoint["label_mean"].cpu(),
        label_std=checkpoint["label_std"].cpu(),
        dataset_name="val_eval",
    )


def build_model(checkpoint: dict, device: torch.device) -> GWFlowModelV3:
    model = GWFlowModelV3(
...
def _get_required_config_value(key: str) -> int:
    ...
    if key not in run_config:
        raise KeyError(f"Missing '{key}' in {RUN_CONFIG_PATH}")
    return int(run_config[key])


def inverse_label_normalization(
