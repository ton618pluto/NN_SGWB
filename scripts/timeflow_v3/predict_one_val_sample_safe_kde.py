...
OUTPUT_NAME = "val_sample_posterior_kde_v3.png"
...
def draw_kde_contour(ax, x: np.ndarray, y: np.ndarray) -> None:
    ...
    try:
        ...
        ax.contourf(grid_x, grid_y, density, levels=6, cmap="Blues", alpha=0.9)
        ax.contour(grid_x, grid_y, density, levels=6, colors="#355c7d", linewidths=0.55)
    except Exception:
        hist, x_edges, y_edges = np.histogram2d(x, y, bins=28, density=True)
        x_centers = 0.5 * (x_edges[:-1] + x_edges[1:])
        y_centers = 0.5 * (y_edges[:-1] + y_edges[1:])
        grid_x, grid_y = np.meshgrid(x_centers, y_centers)
        ax.contourf(grid_x, grid_y, hist.T, levels=6, cmap="Blues", alpha=0.9)
        ax.contour(grid_x, grid_y, hist.T, levels=6, colors="#355c7d", linewidths=0.55)
...
def plot_corner_kde(samples: np.ndarray, true_values: np.ndarray, output_path: Path) -> None:
    ...
    fig.suptitle(
        "Posterior joint distribution (KDE) for one v3 validation sample\n"
