...
OUTPUT_DIR = Path(__file__).resolve().parent / "MAE_fig"
OUTPUT_FIG_NAME = "new_model_param_mae_multi_sample_v3.png"
OUTPUT_TXT_NAME = "new_model_param_mae_multi_sample_v3.txt"
...
def save_metrics_text(
    ...
    lines = [
        f"num_eval_samples: {overall_mae_per_sample.shape[0]}",
        f"overall_mae_mean: {overall_mae_per_sample.mean():.8f}",
        f"overall_mae_std: {overall_mae_per_sample.std():.8f}",
        f"val_nll_mean: {nll_per_sample.mean():.8f}",
        f"val_nll_std: {nll_per_sample.std():.8f}",
        "",
...
def plot_parameter_mae(
    ...
    ax.set_title("V3 model per-parameter MAE on validation split", fontsize=13)
...
def compute_val_nll(model, waveform: torch.Tensor, label_normalized: torch.Tensor, device: torch.device) -> float:
...
def main() -> None:
    ...
    eval_count = min(NUM_EVAL_SAMPLES, len(dataset))
    ...
    progress = tqdm(range(eval_count), desc="Evaluating v3 parameter MAE", unit="sample")
    for sample_index in progress:
        ...
        nll = compute_val_nll(model, waveform, label_normalized, device)
        per_sample_param_maes.append(param_mae)
        overall_maes.append(param_mae.mean())
        nll_values.append(nll)
        progress.set_postfix(overall_mae=f"{np.mean(overall_maes):.2f}", nll=f"{np.mean(nll_values):.2f}")
    ...
    print(f"Validation NLL mean: {nll_values_array.mean():.6f}")
