...
class GWDatasetV3(Dataset):
    ...
    def split_records(
        ...
        if len(records) < 2:
            raise ValueError("At least 2 samples are required to split train/val")
    ...
    def _calculate_label_stats(self) -> tuple[np.ndarray, np.ndarray]:
        ...
        for record in progress:
            try:
                ...
                print(f"[{self.dataset_name}] Failed to load label file: {record['h1']}")
                raise
    ...
    def _load_channel(self, path: Path) -> np.ndarray:
        try:
            ...
            print(f"[{self.dataset_name}] Failed to load channel file: {path}")
            raise
    ...
    def __getitem__(self, index: int) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        try:
            ...
            print(f"[{self.dataset_name}] Failed to load H1 file: {record['h1']}")
            raise
