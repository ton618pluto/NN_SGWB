...
TRAIN_H1_DIR = SCRIPTS_ROOT / "processed_data_train" / "v3" / "training_set0" / "H1_splits"
TRAIN_L1_DIR = SCRIPTS_ROOT / "processed_data_train" / "v3" / "training_set0" / "L1_splits"
USE_L1 = True
TRAIN_SAMPLE_STEP = 1
VAL_FRACTION = 0.2
BATCH_SIZE = 32
...
CHECKPOINT_NAME = "best_flow_v3.pt"
...
def make_loader(dataset: GWDatasetV3, batch_size: int, shuffle: bool, seed: int) -> DataLoader:
...
def save_split_manifest(train_records: list[dict], val_records: list[dict]) -> None:
...
def save_run_config(train_dataset: GWDatasetV3, train_size: int, val_size: int) -> None:
...
@torch.no_grad()
def evaluate(model: GWFlowModelV3, loader: DataLoader, device: torch.device) -> float:
    model.eval()
    total_loss = 0.0
...
def main() -> None:
    ...
    all_records = GWDatasetV3.build_records(
        h1_dir=TRAIN_H1_DIR,
        l1_dir=train_l1_dir,
        sample_step=TRAIN_SAMPLE_STEP,
        dataset_name="full",
    )
    train_records, val_records = GWDatasetV3.split_records(
        records=all_records,
        val_fraction=VAL_FRACTION,
        seed=SEED,
    )

    train_dataset = GWDatasetV3(records=train_records, dataset_name="train")
    val_dataset = GWDatasetV3(
        records=val_records,
    ...
    OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
    save_split_manifest(train_records, val_records)
    save_run_config(train_dataset, train_size=len(train_records), val_size=len(val_records))
