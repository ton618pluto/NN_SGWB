...
if __package__ is None or __package__ == "":
    ...
    from dataset import GWDatasetV4
    from model import GWFlowModelV4
else:
    from .dataset import GWDatasetV4
    from .model import GWFlowModelV4
...
BEST_CHECKPOINT_NAME = "best_flow_v4.pt"
LATEST_CHECKPOINT_NAME = "latest_flow_v4.pt"
...
def make_loader(dataset: GWDatasetV4, batch_size: int, shuffle: bool, seed: int) -> DataLoader:
...
def save_run_config(train_dataset: GWDatasetV4, train_size: int, val_size: int, test_size: int) -> None:
...
@torch.no_grad()
def evaluate(model: GWFlowModelV4, loader: DataLoader, device: torch.device, desc: str) -> float:
    model.eval()
    total_loss = 0.0
...
def main() -> None:
    ...
    all_records = GWDatasetV4.build_records(
        h1_dir=TRAIN_H1_DIR,
        l1_dir=train_l1_dir,
        sample_step=TRAIN_SAMPLE_STEP,
        dataset_name="full",
    )
    train_records, val_records, test_records = GWDatasetV4.split_records(
        records=all_records,
        val_fraction=VAL_FRACTION,
        test_fraction=TEST_FRACTION,
        seed=SEED,
    ...
    train_dataset = GWDatasetV4(records=train_records, dataset_name="train")
    val_dataset = GWDatasetV4(
        records=val_records,
        label_mean=train_dataset.label_mean,
        label_std=train_dataset.label_std,
        dataset_name="val",
    )
    test_dataset = GWDatasetV4(
