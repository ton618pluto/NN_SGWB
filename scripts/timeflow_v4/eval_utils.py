...
CHECKPOINT_PATH = OUTPUT_DIR / "best_flow_v4.pt"
...
def load_records_from_manifest(path: Path) -> tuple[list[dict], list[dict], list[dict]]:
    ...
    test_records = [_deserialize_record(item) for item in manifest["test_records"]]
...
def build_model(checkpoint: dict, device: torch.device) -> GWFlowModelV4:
    model = GWFlowModelV4(
...
def build_split_dataset(checkpoint: dict, split_name: str) -> GWDatasetV4:
    ...
    if split_name not in records_map:
        raise ValueError(f"Unsupported split_name: {split_name}")

    records = records_map[split_name]
    use_l1 = int(checkpoint["num_channels"]) == 2
    if use_l1:
        missing_l1 = [record["h1"] for record in records if record["l1"] is None]
        if missing_l1:
            raise RuntimeError(
                f"{split_name} split manifest does not contain usable L1 paths for a 2-channel model. "
