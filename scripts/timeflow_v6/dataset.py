...
def _filter_label(label: np.ndarray) -> np.ndarray:
    # 原始顺序: [zp, alpha_z, beta_z, alpha_m, m_max, delta_m, m_min, lambda_peak, mu_m, sigma_m, beta_q]
    # v6 去掉 alpha_z 和 beta_z，仅保留 zp + 其余独立目标。
    return np.concatenate([label[:1], label[3:]]).astype(np.float32, copy=False)


class GWDatasetV6(Dataset):
    def __init__(
        self,
