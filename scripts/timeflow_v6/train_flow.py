...
BEST_CHECKPOINT_NAME = "best_flow_v6.pt"
LATEST_CHECKPOINT_NAME = "latest_flow_v6.pt"
...
def make_loader(dataset: GWDatasetV6, batch_size: int, shuffle: bool, seed: int) -> DataLoader:
...
def save_run_config(train_dataset: GWDatasetV6, train_size: int, val_size: int, test_size: int) -> None:
...
def train_epoch(model: GWFlowModelV6, loader: DataLoader, optimizer: optim.Optimizer, scaler: GradScaler, device: torch.device) -> float:
    model.train()
    total_loss = 0.0
...
@torch.no_grad()
def evaluate(model: GWFlowModelV6, loader: DataLoader, device: torch.device, desc: str) -> float:
    model.eval()
    total_loss = 0.0
...
def main() -> None:
    ...
    all_records = GWDatasetV6.build_records(TRAIN_H1_DIR, train_l1_dir, sample_step=TRAIN_SAMPLE_STEP, dataset_name="full_superimposed")
    train_records, val_records, test_records = GWDatasetV6.split_records(all_records, VAL_FRACTION, TEST_FRACTION, SEED)

    train_dataset = GWDatasetV6(records=train_records, dataset_name="train_superimposed")
    val_dataset = GWDatasetV6(records=val_records, label_mean=train_dataset.label_mean, label_std=train_dataset.label_std, dataset_name="val_superimposed")
    test_dataset = GWDatasetV6(records=test_records, label_mean=train_dataset.label_mean, label_std=train_dataset.label_std, dataset_name="test_superimposed")
