...
CHECKPOINT_PATH = OUTPUT_DIR / "best_flow_v8.pt"
...
def build_model(checkpoint: dict, device: torch.device) -> GWFlowModelV8:
    model = GWFlowModelV8(
...
def build_split_dataset(checkpoint: dict, split_name: str) -> GWDatasetV8:
...
def sample_posterior(model: GWFlowModelV8, waveform: torch.Tensor, num_samples: int, device: torch.device) -> torch.Tensor:
